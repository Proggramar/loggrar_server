...
import { CurrencyCreateDto, CurrencyUpdateDto } from './dto';
import { CurrencyService } from './currency.service';
import { CurrencySetting } from './entities/currency.entity';

@ApiTags('Currencies')
@ApiBearerAuth()
@Controller('loggrar/Setting/Currency')
export class CurrencyController {
  constructor(private controllerService: CurrencyService) {}

  @ApiOperation({ summary: 'Get currencies for grid view.', description: 'Get currencies for grid view.' })
  ...
  @ApiOperation({ summary: 'List of currencies', description: 'Get all currencies' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Process OK.' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Unauthorized information.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Bad request.' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized.' })
  ...
  @ApiOperation({ summary: 'Create a currency', description: 'Create a new currency' })
  ...
  @Auth({ roles: [ValidRoles.super, ValidRoles.system, ValidRoles.administrator, ValidRoles.basic] })
  @Post()
  async create(@Body() body: CurrencyCreateDto): Promise<TypeResponse> {
    const record: any = await this.controllerService.create(body);
    return toBackResponse('Record created successfully');
  }

  @ApiOperation({ summary: 'Get a currency', description: 'Get a currency by your id' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Process OK.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Bad request.' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Unauthorized information.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Bad request.' })
  ...
  async get(@Param('id', ParseUUIDPipe) id: string): Promise<TypeResponse> {
    ...
    return toBackResponse('Record returned', { records: plainToInstance(CurrencySetting, data) });
  }

  @ApiOperation({ summary: 'Update a currency', description: 'Update a currency by your id' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Process OK.' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Unauthorized information.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Bad request.' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized.' })
  ...
  @ApiOperation({ summary: 'Delete a currency', description: 'Delete a currency by your id' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Process OK.' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Unauthorized information.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Bad request.' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized.' })
